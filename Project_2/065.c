...
void* AccessMemory(uint32_t Phy_addr, int pte_bit)
{
    ...
    if(pte_bit == 1)
    {
        ...
        int idx_offset = idx % 256;
        int idx_entry = idx / 256;
        // return a pointer to the value to be type casted later
        return &(main_memory[0].pf.pages[idx_entry].page_table[idx_offset]);
    }
    else if(pte_bit == 2)
    {
        // if page directory is accessed
        int idx = (Phy_addr/(sizeof(struct PDE)));
        return &(main_memory[0].pf.pages[4].page_directory[idx]);
    }
    // else page is accessed and returned
    int pf_no = (Phy_addr & PFN_MASK) >> PFN_SHIFT;
    int offset = (Phy_addr & OFFSET_MASK);
    // return a pointer to the value to be type casted later
    return &(main_memory[pf_no].pf.pages[offset].data);
}
...
uint32_t convert_to_physical(uint32_t virtual_addr)
{
    ...
    tlb_miss_start = clock(); //start the clock

    uint32_t PDIndex = (VPN & PD_MASK) >> PD_SHIFT;
    uint32_t PDEAddr = (PDIndex * sizeof(struct PDE));
    struct PDE * page_directory_entry = (struct PDE *)AccessMemory(PDEAddr, 2);
    if (page_directory_entry->valid == 0){
        // if pde valid bit is false
        printf("\n\n**SEGMENTATION_FAULT**\n\n");
        return -1;
    }
    // printf("pde done\n");
    // else access the Page table entry
    // calculate PTE_Addr
    uint32_t PTE_ADDR = (VPN*sizeof(struct PTE));

    // calculate page table entry
    struct PTE* Page_table_entry = (struct PTE *) AccessMemory(PTE_ADDR, 1);
    // printf("page table entry done\n");
    // if not valid
...
void populate_page_table(int n)
{
    ...
    for(int i=0; i<4; i++)
    {
        // will store 256 entries in a single page (since 256*4 = 1024)
        main_memory[0].pf.pages[i].page_table = (struct PTE*)(malloc(sizeof(struct PTE)*256));
        // now populate each of the 256 entries
        for(int j=0;j<256;j++)
        {
            main_memory[0].pf.pages[i].page_table[j].PFN = 1 + (rand()%(1<<13));
            // setting up valid bit 0 with 0.005 probability 
            // or the PTE are lying in pages 3,4 (empty since only 512 entries)
            if(rand()%500 == 0 || (i>1)){
                main_memory[0].pf.pages[i].page_table[j].valid = 0;
            }
            else{
                main_memory[0].pf.pages[i].page_table[j].valid = 1; 
            }
             // initially bits are not dirty
            main_memory[0].pf.pages[i].page_table[j].dirty = 0;

            // assuming all are in user mode
            main_memory[0].pf.pages[i].page_table[j].mode = 0;

            // assuming enough memmory for this particular process
            main_memory[0].pf.pages[i].page_table[j].present = 1;

            main_memory[0].pf.pages[i].page_table[j].protected = 0;
            // adding read/ write and execute protected with probability 0.01
            if(rand()%100 == 0){
                main_memory[0].pf.pages[i].page_table[j].protected += 1;
            }
            if(rand()%100 == 0){
                main_memory[0].pf.pages[i].page_table[j].protected += (1<<1);
            }
            if(rand()%100 == 0){
                main_memory[0].pf.pages[i].page_table[j].protected += (1<<2);
            }
        }
    }
}
...
void populate_page_directory(){
    srand(time(NULL));
    // allocate a 4 size directory in the 5th page
    main_memory[0].pf.pages[4].page_directory = (struct PDE*)(malloc(sizeof(struct PTE)*4));
    for(int i=0;i<4;i++)
    {
        main_memory[0].pf.pages[4].page_directory[i].PFN = i;
        // only first two directories will have entries
        if(i<2) main_memory[0].pf.pages[4].page_directory[i].valid = 1;
        else main_memory[0].pf.pages[4].page_directory[i].valid = 0;
    }
}
