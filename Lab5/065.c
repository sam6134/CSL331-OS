    ...
    if (pid == 0) {
       ...
       exit(1);
        ...
        if(background == 1){
            ...
            if(background == 1 ) printf("Process running with pid %d running in background\n", pid);
        }
    }
}
...
{
    /*
        Execute the commands by splitting them into a pipe.
        Args -> arguments, background bit, argument Size
    */
    int numPipes = cntPipes(args);
    int inputStream, en;
    // declare pipes for each command
    int p[numPipes*2];
    ...
    int st=-1;
    int commandNumber = 0;
    // execute each command with appropriate I/O Streams
    for(int i=0; i<numPipes; i++)
    {
        pipe(p);
        ...
        char* temp = args[en];
        args[en] = NULL;
        executeCommand(args+st+1,background,en-st-1,inputStream,p[commandNumber*2+1]);
        args[en] = temp;
        inputStream = p[commandNumber*2];
        st = en;
    }
    ...
    // wait for each child process spawned
    int status;
    if(background != 1)
